...
# def find_best_overall_combinations(all_combinations, top_n=20):
#     """
#     全体で最も効率的な組み合わせを表示
#     """
#     print(f"\n全体での最適組み合わせ（余りの少ない順 上位{top_n}個）:")
#     print("=" * 70)
    
#     # 余りの少ない順でソート
#     best_combinations = sorted(all_combinations, key=lambda x: x['waste'])
    
#     for i, combo in enumerate(best_combinations[:top_n]):
#         cuts_str = ' + '.join(map(str, combo['cuts']))
#         print(f"{i+1:2d}. 棒{combo['rod_length']}mm: {cuts_str} = {combo['total_used']}mm "
#               f"(余り: {combo['waste']}mm, 効率: {combo['efficiency']:.1f}%)")
...
def select_cutting_plan(all_combinations, required_cuts):
    ...
    while any(qty > 0 for qty in remaining.values()):
        ...
        for combo in sorted_combinations:
            # 既に選択された組み合わせはスキップしているがその必要はないはず
            # if combo in [plan['combination'] for plan in selected_plan]:
            #     continue
            ...
            combined_score = score - (combo['waste'] * 0.001)
            
            if combined_score > best_score and score > 0:
...
if __name__ == "__main__":
    ...
    # 最適な組み合わせを表示
    # find_best_overall_combinations(all_combinations)
