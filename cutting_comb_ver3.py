...
def select_cutting_plan(all_combinations, required_cuts):
    ...
    # scoreのペナルティ設定
    over_penalty = 0.01  # 余分な切り出しに対するペナルティ
    waste_penalty = 0.015  # ロス長に対するペナルティ
...
if __name__ == "__main__":
    ...
    if desplay > 0:
        ...
        # print(f"組み合わせ探索時間: {end - start:.2f}秒")

    # あるケースでの切り出しプランのロス率
    start = time.perf_counter()
    select_cutting_plan(all_combinations, required_cuts)
    end = time.perf_counter()
    # print(f"\n切り出しプラン選択時間: {end - start:.2f}秒")
